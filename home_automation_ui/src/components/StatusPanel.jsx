...
const StatusPanel = () => {
  ...
  const loadAllData = useCallback(async () => {
    ...
    const [statusData, aqiData, astronomyData] = await Promise.all([
      fetchWithRetry("http://localhost:3000/api/status"),
      fetchWithRetry("http://localhost:3000/api/aqi"),
      fetchWithRetry("http://localhost:3000/api/astronomy")
    ]);
  ...
  useEffect(() => {
    ...
    if (locationPref) {
      ...
      navigator.geolocation.getCurrentPosition(
        async ({ coords }) => {
          try {
            const rev = await fetch("http://localhost:3000/api/reverselocation", {
              method: "POST",
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify({
                lat: coords.latitude,
          ...
          <button
            onClick={async () => {
              ...
              try {
                ...
                const geo = await fetch("http://localhost:3000/api/geolocation", {
                  method: "POST",
                  headers: { "Content-Type": "application/json" },
                    ...
                    return (
                      <div className="flex-1 overflow-hidden min-h-0">
                        ...
                        ease-in-out hover:scale-105 hover:shadow-xl hover:-translate-y-1 cursor-auto">
                            ...
                            <p className="text-lg text-center mt-1">Change parameters of the fields to get desired results.Coming soon...</p>
                          </div>
                        </div>
                      </div>
